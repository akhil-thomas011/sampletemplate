...
function validateTemplate(requestBody, templateFilePath) {
  var validatePromise;
  console.log('Custom log: Entered validateTemplate');
      ...
      unirest.post(process.env.VALIDATION_HOST + '/validate')
        ...
        .end(function (response) {
          if (response.status !== 200) {
            ...
            console.log('Custom log: Validation Succesfull. Exiting validateTemplate');
...
function deployTemplate(requestBody) {
  var deployPromise;
  console.log('Custom log: Entered deployTemplate');
      ...
      return unirest.post(process.env.VALIDATION_HOST + '/deploy')
        ...
        .end(function (response) {
          ...
          if (response.body.result === 'Deployment Successful') {
            console.log('Custom log: Deployment Successful.Exiting deployTemplate');
            resolve(response.body);
          } else {
            reject(response.body);
          }
